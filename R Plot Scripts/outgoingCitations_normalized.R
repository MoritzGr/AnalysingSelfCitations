...
p <- ggplot(table, aes(x = value, y = (selfCitingPaperCount-selfCitingDuplicatePaperCount)/((paperCount-duplicatePaperCount)*value))) + geom_line(aes(color = "Normalized Frequency of Self-Citations [% of Papers]")) + ylab("")+ scale_y_continuous(labels = scales::percent)
